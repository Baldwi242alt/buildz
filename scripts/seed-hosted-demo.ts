...
export async function seedHostedDemo(client:pg.Client) {
 ...
 try {
  ...
  for(let i=0;i<3;i++)await client.query("insert into app.collaboration_requests(id,project_id,requester_id,message,kind) values($1,$2,$3,'Demo collaborator: I would like to help test the next prototype. Could we discuss joining?','join') on conflict(id) do nothing",[id(5100+i),id(10+i),id(5)]);
  ...
  // Fictional accepted invitation history, before demo ownership transferred.
  // This does not add membership or silently accept a live person's request.
  await client.query("insert into app.project_invitations(id,project_id,recipient_email,role,invited_by,state,accepted_by,expires_at) values($1,$2,'demo@buildz.example','editor',$3,'accepted',$4,$5) on conflict(id) do nothing",[id(5201),id(10),id(3),owner,at(30,18)]);
